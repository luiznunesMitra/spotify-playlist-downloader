...
e baixa em MP3 320kbps usando yt-dlp.

Uso:
    python spotify_dl.py <URL_DA_PLAYLIST> [--output PASTA] [--format mp3|flac|opus]
    python spotify_dl.py <URL_DA_PLAYLIST> --list-only   # só lista as músicas, não baixa

Requisitos:
    pip install yt-dlp
    sudo apt install ffmpeg   (ou equivalente no seu OS)
"""
...
def get_embed_token_and_first_100(playlist_id):
    ...
    if not token_match:
        print("ERRO: N��o foi possível obter token do Spotify.")
        sys.exit(1)
    token = token_match.group(1)

    # Extrair tracks do __NEXT_DATA__
    nd = re.search(
    ...
    if not nd:
        print("ERRO: Não foi possível extrair dados da playlist.")
        sys.exit(1)
...
def get_playlist_tracks(playlist_id):
    ...
    if all_uris and len(all_uris) > len(first_100):
        ...
        print("[3/3] Todas as faixas já foram obtidas do embed.")
...
def save_tracklist(tracks, output_path):
    """Salva a lista de faixas legível."""
    with open(output_path, "w", encoding="utf-8") as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="Baixa músicas de playlists do Spotify via YouTube",
        epilog="Exemplo: python spotify_dl.py https://open.spotify.com/playlist/XXXXX",
    )
    parser.add_argument("url", help="URL da playlist do Spotify")
    parser.add_argument(
        "--output", "-o",
        default=None,
        help="Pasta de saída (padrão: nome da playlist)",
    )
    parser.add_argument(
        "--format", "-f",
        default="mp3",
        choices=["mp3", "flac", "opus", "m4a", "wav"],
        help="Formato do áudio (padrão: mp3)",
    )
    parser.add_argument(
        "--list-only", "-l",
        action="store_true",
        help="Apenas lista as faixas, não baixa",
    )
    parser.add_argument(
        "--batch-only", "-b",
        action="store_true",
        help="Apenas gera o arquivo batch (download_all.txt), não baixa",
    )
    ...
    # Limpar nome pra usar como pasta
    ...
    if args.batch_only:
        ...
        print(f"  yt-dlp --batch-file {batch_path} -x --audio-format mp3 --audio-quality 0 -o '%(title)s.%(ext)s'")
        return

    # Verificar dependências
    for cmd in ["yt-dlp", "ffmpeg"]:
        if subprocess.run(["which", cmd], capture_output=True).returncode != 0:
            # Tenta 'where' no Windows
            if subprocess.run(["where", cmd], capture_output=True, shell=True).returncode != 0:
                print(f"\nERRO: '{cmd}' não encontrado. Instale com:")
                print(f"  sudo apt install {cmd}   (Linux)")
                print(f"  brew install {cmd}        (macOS)")
                print(f"  winget install {cmd}      (Windows)")
                sys.exit(1)
    ...
    print(f"\nConcluído! {count} arquivos baixados em: {output_dir}")
